...
declare
  ...
  v_fuel_distance AIRPLANES_TYPES.FUEL_DISTANCE%type;
...
begin
  ...
  loop
    begin
      ...
      v_index := instr(v_line, ',');
      v_model := trim(substr(v_line, 0, v_index - 1));
      v_line := trim(substr(v_line, v_index + 1));
      v_index := instr(v_line, ',');
      v_capacity := trim(substr(v_line, 0, v_index - 1));
      v_line := trim(substr(v_line, v_index + 1));
      v_index := instr(v_line, ',');
      v_fuel_distance := trim(substr(v_line, 0, v_index -1 ));
      v_line := trim(substr(v_line, v_index + 1));
      v_index := instr(v_line, ',');
      v_speed := trim(substr(v_line, 0, v_index - 1));
      v_fuel_economy := trim(substr(v_line, v_index + 1));

      --DBMS_OUTPUT.put_line(v_model || ' ' || v_capacity || ' ' || v_fuel_distance || ' ' || v_speed || ' ' || v_fuel_economy);

      insert into AIRPLANES_TYPES(MODEL, SPEED, FUEL_DISTANCE, FUEL_ECONOMY, CAPACITY) values(v_model, v_speed, v_fuel_distance, v_fuel_economy, v_capacity);
...
--tabel cu rownum si index pe el
--select bulk pt companies
--for each company: generate 5 numbers and add 5 airplanes for current company
declare
